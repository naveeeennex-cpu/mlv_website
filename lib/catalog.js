...
export const BUDGET_BANDS = [
  ...
  { id: 'bud:2', label: '₹15,000 – ₹35,000', min: 15000, max: 35000 },
  { id: 'bud:3', label: 'Above ₹35,000',     min: 35000, max: null },
];
...
// All sliding door locks, regardless of material (Wooden or Metal/aluminium).
// Used for the sliding flow, where the small fixed set is shown for any door type.
export async function slidingProducts(limit = 5) {
  const db = getDb();
  const { rows } = await db.query(
    `SELECT * FROM products
     WHERE is_active = true AND in_stock IS NOT FALSE
       AND category_id = ANY($1)
       AND door_configuration @> '["Sliding Door"]'::jsonb
     ORDER BY price ASC LIMIT ${Number(limit)}`,
    [DOOR_LOCK_CATEGORIES]
  );
  return rows.map(shapeProduct);
}
