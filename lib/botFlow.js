...
// Short, simple intro shown at the start of every conversation.
const ABOUT_MLV =
  `👋 Welcome to *MLV Enterprises* — your trusted *Yale Authorised Dealer* in Chennai.\n\n` +
  `We supply and *install* smart door locks, digital locks, safes and security products for ` +
  `homes, apartments and offices — with free installation and full after-sales support.`;
...
export async function runBotFlow(input, client, session, from, log) {
  ...
  if (bot.step === 'start') {
    ...
    if (product) {
      ...
      await s.text(ABOUT_MLV);
      await s.text(
        `You're enquiring about *${product.name}*.\n\n` +
        `Before we proceed, let's make sure it's compatible with your door.`
      );
      return askMaterial(s, bot, false);
    }
    bot.flow = 'meta';
    await s.text(ABOUT_MLV);
    await s.text(`Let's find the right product for you.`);
  ...
  switch (bot.step) {
    ...
    case 'await_wardrobe_type': return onWardrobeType(s, bot, id, text);
...
async function onLocation(s, bot, client, from, id, text) {
  ...
  if (bot.flow === 'website') {
    ...
    return proceedToProducts(s, bot);
  }
...
// Wooden doors have many locks → ask budget to narrow. Metal / Glass / uPVC have
// only one each, so skip the budget step and show the product(s) directly.
async function proceedToProducts(s, bot) {
  if (bot.doorMaterial === 'Wooden Door') return askBudget(s, bot);
  return showMatches(s, bot, await searchCompatible(bot.doorMaterial, bot.doorConfiguration));
}
...
async function onSelection(s, bot, id, text) {
  ...
  // Image as the card header → photo shows first, then description, then buttons,
  // all in one message (avoids the image arriving after the text).
  await s.buttons({
...
async function onCategory(s, bot, id, text) {
  ...
  // Wardrobe splits into Openable vs Sliding before showing products.
  if (key === 'wardrobe') return askWardrobeType(s, bot);
  return showMatches(s, bot, await categoryProducts(key));
}

async function askWardrobeType(s, bot) {
  bot.step = 'await_wardrobe_type';
  await s.buttons({
    body: 'What type of wardrobe do you have?',
    buttons: [
      { id: 'ward:openable', title: 'Openable Wardrobe' },
      { id: 'ward:sliding', title: 'Sliding Wardrobe' },
    ],
  });
}

async function onWardrobeType(s, bot, id, text) {
  let type = id === 'ward:openable' ? 'openable' : id === 'ward:sliding' ? 'sliding' : null;
  if (!type && /openable/i.test(text)) type = 'openable';
  if (!type && /sliding/i.test(text)) type = 'sliding';
  if (!type) return nudge(s, () => askWardrobeType(s, bot));
  const all = await categoryProducts('wardrobe', 20);
  const list = all.filter(p => type === 'sliding'
    ? p.doorConfiguration.includes('Sliding Door')
    : !p.doorConfiguration.includes('Sliding Door'));
  return showMatches(s, bot, list.slice(0, 10));
}
