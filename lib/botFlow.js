...
import {
  DOOR_MATERIALS, DOOR_CONFIGS, DOOR_LOCATIONS, OTHER_CATEGORIES,
...
function freshBot() {
  return { flow: null, step: 'start', productId: null, productName: null,
    doorMaterial: null, doorConfiguration: null, doorLocation: null, candidates: [] };
}
...
async function onLocation(s, bot, client, from, id, text) {
  ...
  if (bot.flow === 'website') {
    ...
    // Incompatible → explain, then offer alternatives.
  ...
  // Meta flow → search by material + configuration.
  return showMatches(s, bot, await searchCompatible(bot.doorMaterial, bot.doorConfiguration));
}
...
async function showMatches(s, bot, products) {
  ...
  if (!products.length) {
    await s.text(
      ...
      `${bot.doorConfiguration && bot.doorConfiguration !== 'Others' ? ` / ${bot.doorConfiguration}` : ''} right now.\n\n` +
