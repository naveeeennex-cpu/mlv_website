...
// List-row subtitle (plain text — WhatsApp lists don't render strikethrough).
function priceLabel(p) {
  const { price, mrp, hasDisc, off } = priceBits(p);
  return hasDisc ? `${inr(price)} · MRP ${inr(mrp)} · ${off}% off` : inr(price);
}

// Card price line (message body — supports ~strikethrough~).
function priceLine(p) {
  const { price, mrp, hasDisc, off } = priceBits(p);
  return hasDisc ? `Price: *${inr(price)}*  ~${inr(mrp)}~  (${off}% OFF)` : `Price: *${inr(price)}*`;
}
