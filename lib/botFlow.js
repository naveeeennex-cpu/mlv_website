...
const WELCOME =
  ...
  `We supply and install Yale smart locks, digital locks, safes and security products for homes, apartments and offices.\n\n` +
  `Not sure which lock to choose? No worries 👍\n` +
  `We'll help you find the right lock for your door and budget.\n\n` +
  `🚚 Free Delivery\n🔧 Free Installation\n🤝 After-Sales Support`;
...
const LOCATIONS = [
  ...
  { id: 'lo:bedroom', title: '🛏️ Bedroom',   value: 'Bedroom / Master Bedroom' },
...
function productDetailText(p) {
  const lines = [`🔐 *Yale ${p.name}*`, `💰 ${priceLabel(p)}`];
...
export async function runBotFlow(input, client, session, from, log) {
  ...
  if (bot.step === 'start') {
    ...
    if (product) {
      ...
      await s.text(
        `👋 Welcome to *MLV Enterprises*.\n\n` +
        `You're enquiring about *${product.name}*.\n` +
        `Let's make sure it fits your door 👇`
      );
...
async function askDoorType(s, bot) {
  ...
  await s.buttons({ body: 'First, please select your door type 👇', buttons: doorTypeButtons });
}
...
async function askBudget(s, bot) {
  bot.step = 'ask_budget';
  await s.buttons({ body: 'What is your budget for the smart lock? 💰', buttons: budgetButtons });
}
...
async function validateWebsite(s, bot, client, from) {
  ...
  if (product && isCompatible(product, bot.materials, bot.doorStyle)) {
    ...
    bot.booked = true;
    await s.text(
      `✅ Great! This lock fits your door.\n\n` +
      `Your enquiry is registered. Our MLV Enterprises team will contact you shortly with price, free installation and availability. 🔧`
    );
    bot.step = 'done';
    return;
  }
  ...
  await s.text(
    ...
    `Let us show you the right locks 👇`
  );
...
async function recommend(s, bot, client, from) {
  ...
  if (!products.length) {
    ...
    await s.buttons({
      body: `We couldn't find an exact match for your selection.\n\nOur lock expert will help you choose the best option. 👍`,
  ...
  await s.list({
    header: 'Best Matches',
    body: `Here are the best locks for your *${bot.doorType}* 👇\nTap one to see details.`,
...
async function onAction(s, bot, client, from, id, text) {
  if (id === 'buy:now' || /buy/i.test(text)) {
    ...
    bot.step = 'done';
    await s.text(
      `🛒 Thank you! Your order enquiry for *${bot.selectedName}* is registered.\n\n` +
      `Our team will contact you shortly to confirm price, delivery and free installation. 🔧🙏`
    );
    return;
  }
  ...
  if (id === 'talk:expert' || /expert|talk|call/i.test(text)) {
    ...
    bot.step = 'done';
    await s.text(
      `📞 Sure! Our lock expert will contact you shortly to help you choose the right lock.\n\n` +
...
async function reshowCandidates(s, bot) {
  ...
  await s.list({
    header: 'Best Matches',
    body: 'Tap a product to see details 👇',
    buttonText: 'View products',
