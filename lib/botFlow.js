...
import {
  BUDGET_BANDS,
  findProductByMessage, getProductById, isCompatible, searchCompatible, slidingProducts,
} from './catalog.js';
...
const WELCOME =
  `Welcome to *MLV Enterprises*, an Authorised Yale product Dealer.\n\n` +
...
// Selling price, MRP and discount % (MRP parsed from the stored "₹X,XXX" string).
function priceBits(p) {
  const price = Number(p.price) || 0;
  const mrp = p.mrp ? Number(String(p.mrp).replace(/[^0-9]/g, '')) || 0 : 0;
  const hasDisc = mrp > price && price > 0;
  return { price, mrp, hasDisc, off: hasDisc ? Math.round((mrp - price) / mrp * 100) : 0 };
}

// List-row subtitle (plain text, single line — WhatsApp lists can't strike/bold).
// Show only the offer price + discount so it's not confused with the MRP; the
// full MRP breakdown appears on the product card.
function priceLabel(p) {
  const { price, hasDisc, off } = priceBits(p);
  return hasDisc ? `${inr(price)} · ${off}% OFF` : inr(price);
}

// Card price block (message body — supports ~strikethrough~ and line breaks).
function priceLine(p) {
  const { price, mrp, hasDisc, off } = priceBits(p);
  if (!hasDisc) return `Your Price: *${inr(price)}*`;
  return `MRP: ~${inr(mrp)}~\nDiscount: *${off}% OFF*\nYour Price: *${inr(price)}*`;
}

// Compact product card (image sent as the card header separately).
function productCard(p, bot) {
  const lines = [`*Yale ${p.name}*`, '', priceLine(p), ''];
...
export async function runBotFlow(input, client, session, from, log) {
  ...
  // Honour a recognised selection button even if tapped out of order — WhatsApp
  // keeps old buttons tappable, so a correction (e.g. Single after Sliding) works.
  if (id) {
    const dt = DOOR_TYPES.find(d => d.id === id);
    if (dt) { bot.doorType = dt.label; bot.materials = dt.materials; bot.needBudget = dt.budget; return askDoorStyle(s, bot); }
    const st = DOOR_STYLES.find(d => d.id === id);
    if (st) { bot.doorStyle = st.value; return askLocation(s, bot); }
    const lo = LOCATIONS.find(l => l.id === id);
    if (lo) {
      if (lo.value === 'Other') {
    ...
    const bud = BUDGET_BANDS.find(x => x.id === id);
    if (bud) { bot.budget = { min: bud.min, max: bud.max, label: bud.label }; return recommend(s, bot, client, from); }
  }
...
// Budget is only useful when there are many products: Wooden AND not Sliding.
// Sliding has just a few locks, so skip budget and show them directly.
const wantsBudget = (bot) => bot.needBudget && bot.doorStyle !== 'Sliding Door';

async function afterLocation(s, bot, client, from) {
  if (bot.flow === 'website') return validateWebsite(s, bot, client, from);
  return proceed(s, bot, client, from);
}

// Decide what to show once door type / style / location are known.
async function proceed(s, bot, client, from) {
  if (bot.doorStyle === 'Sliding Door') return recommendSliding(s, bot, client, from);
  if (wantsBudget(bot)) return askBudget(s, bot);
  return recommend(s, bot, client, from);
}

// Sliding: show the fixed set of sliding locks for Wooden & Metal/uPVC.
// Glass sliding has no product yet → "coming soon".
async function recommendSliding(s, bot, client, from) {
  if (bot.doorType === 'Glass Door') {
    await upsertLead(client, bot, leadFields(bot, from, bot.flow, null, null, 'Coming Soon'), true);
    bot.booked = true;
    bot.step = 'done';
    await s.text(
      `Sliding locks for *glass doors* are *coming soon*.\n\n` +
      `Our team will contact you shortly with the best available options. Thank you for your interest in MLV Enterprises.`
    );
    return;
  }
  const products = await slidingProducts();
  bot.candidates = products.map(p => p.id);
  bot.step = 'await_selection';
  await s.list({
    header: 'Sliding Door Locks',
    body: 'Here are our sliding door locks. Tap one to see the details.',
...
async function validateWebsite(s, bot, client, from) {
  ...
  if (product && isCompatible(product, bot.materials, bot.doorStyle)) {
    await upsertLead(client, bot, leadFields(bot, from, 'website', product.id, product.name), true);
    bot.booked = true;
    await s.text(
...
async function recommend(s, bot, client, from) {
  // Show all matching products in the band (WhatsApp lists allow up to 10 rows).
  const products = await searchCompatible(bot.materials, bot.doorStyle, bot.budget, 10);
  bot.candidates = products.map(p => p.id);

  if (!products.length) {
    // Don't guess — hand off to a human.
    await upsertLead(client, bot, leadFields(bot, from, bot.flow, null, null, 'Needs Expert'), true);
    bot.booked = true;
...
async function onSelection(s, bot, id, text) {
  ...
  await s.buttons({
    ...
    buttons: cardButtons(bot),
  });
}

// Card action buttons. "More Products" only appears when there are other options.
function cardButtons(bot) {
  const btns = [{ id: 'buy:now', title: 'Book Now' }];
  if ((bot.candidates || []).length > 1) btns.push({ id: 'more:list', title: 'More Products' });
  btns.push({ id: 'talk:expert', title: 'Talk to Expert' });
  return btns;
}

async function onAction(s, bot, client, from, id, text) {
  if (id === 'buy:now' || /buy/i.test(text)) {
    await upsertLead(client, bot, leadFields(bot, from, bot.flow, bot.selectedId, bot.selectedName), true);
    bot.booked = true;
    bot.step = 'done';
    await s.text(
  ...
  if (id === 'more:list' || /more|other/i.test(text)) {
    return reshowCandidates(s, bot);
  }
  ...
  if (id === 'talk:expert' || /expert|talk|call/i.test(text)) {
    await upsertLead(client, bot, leadFields(bot, from, bot.flow, bot.selectedId, bot.selectedName, 'Needs Expert'), true);
    bot.booked = true;
    bot.step = 'done';
    await s.text(
  ...
  return nudge(s, async () => {
    const p = await getProductById(bot.selectedId);
    await s.buttons({ body: productCard(p, bot), imageUrl: p.image, buttons: cardButtons(bot) });
  });
}
...
// Fields for the running (incomplete) conversation — saved on every step so
// partial leads are captured. Used by the handler after each message.
export function partialFields(bot, from) {
  return leadFields(bot, from, bot.flow, bot.selectedId, bot.selectedName, 'In Progress');
}
