...
export async function upsertLead(client, bot, fields, notify = false) {
  ...
  // Alert the owner once when the enquiry starts, and again when it converts.
  if (notify) await notifyOwner(client, fields, leadId, 'completed');
  else if (created) await notifyOwner(client, fields, leadId, 'started');
  return leadId;
}
...
async function notifyOwner(client, f, leadId, kind = 'completed') {
  try {
    const ownerPhone = await getConfig('owner_phone', client.ownerPhone);
    if (!ownerPhone) return;
    const lines = [
      kind === 'started' ? '🟡 *New Enquiry Started*' : '🔔 *Enquiry Completed*',
      `Customer: wa.me/${f.phone}`,
