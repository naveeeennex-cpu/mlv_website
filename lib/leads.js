    ...
    const { rows } = await db.query(
      `INSERT INTO leads
        (lead_source, phone, customer_name, product_id, product_name,
         door_material, door_configuration, door_location, conversation_history, status)
...
async function notifyOwner(client, f, leadId) {
  try {
    ...
    const lines = [
      ...
      f.doorConfiguration ? `Config: ${f.doorConfiguration}` : null,
      f.doorLocation ? `Location: ${f.doorLocation}` : null,
      leadId ? `\nLead #${leadId} · Status: New Lead` : null,
    ].filter(Boolean);
